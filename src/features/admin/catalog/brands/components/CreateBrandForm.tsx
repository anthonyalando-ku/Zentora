...
export const CreateBrandForm = ({
  ...
  onSubmit: (values: CreateBrandValues) => Promise<void> | void;
  isSubmitting: boolean;
}) => {
  const { register, handleSubmit, formState } = useForm<CreateBrandValues>({
    defaultValues: { name: "", slug: "", logo_url: "" },
  });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        <label className="text-xs font-medium text-foreground/60">Logo URL (optional)</label>
        <input
          className="mt-1 w-full h-11 rounded-xl border border-border bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
          {...register("logo_url")}
          placeholder="https://…"
