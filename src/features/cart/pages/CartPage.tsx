    ...
    <MainLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-bold">Shopping Cart ({items.length} item{items.length !== 1 ? "s" : ""})</h1>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {items.map((item) => (
              <div key={item.product.id} className="flex gap-4 p-4 bg-background rounded-2xl border border-border">
                <Link to={`/products/${item.product.slug}`} className="shrink-0 w-24 h-24 rounded-xl overflow-hidden bg-gray-50">
                  <img src={item.product.thumbnail} alt={item.product.name} className="w-full h-full object-cover" />
                </Link>
                <div className="flex-1 min-w-0">
                  <Link to={`/products/${item.product.slug}`} className="font-medium text-sm hover:text-primary line-clamp-2 block mb-1">
                    {item.product.name}
                  </Link>
                  <p className="text-xs text-foreground/40 capitalize mb-2">{item.product.category}</p>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      {item.quantity > 1 && (
                        <div className="text-xs text-foreground/40">KSh {item.product.price.toLocaleString()} each</div>
                      )}
                    </div>
                  </div>
                </div>
